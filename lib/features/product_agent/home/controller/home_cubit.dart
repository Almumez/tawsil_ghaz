...
class ProductAgentHomeCubit extends Cubit<ProductAgentHomeState> {
  ProductAgentHomeCubit() : super(ProductAgentHomeState());
  ...
  Future<void> fetchOrders({bool isPagination = false}) async {
    ...
    try {
      final result = await ServerGate.i.getFromServer(
        url: 'product-agent/order',
        params: {'status': 'new', 'page': currentPage},
      );
